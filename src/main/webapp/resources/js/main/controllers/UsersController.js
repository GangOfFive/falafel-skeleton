...
var UsersController = function($scope, $http, $location, $modal, $log) {
	...
	jQuery(grid_selector).jqGrid({
		...
		colNames : [ 'ID', 'Tipo Usuario', 'Email', 'Name', 'Last Name' ],
		colModel : [{
			name : 'idUsuario',
			hidden: true
		}, {
	...
	// CUSTOM ACTIONS
	$("#add_usersList .ui-pg-div, #edit_usersList .ui-pg-div").click(
			function(ev) {
				ev.preventDefault();
				return false;
			});
	...
	$scope.edit = function() {
		var modalInstance = $modal.open({
			templateUrl : 'layoutservice/user/editUserModal',
			controller : ModalInstanceCtrl,
			resolve : {
				user : function() {
					var row = jQuery(grid_selector).jqGrid('getGridParam', 'selrow'),
						user = jQuery(grid_selector).jqGrid('getRowData', row);
	...
	$("#edit_usersList .ui-pg-div").click(function(ev) {
		$("#openEditUserModal").click();
	});
};

var ModalInstanceCtrl = function($http, $scope, $modalInstance, user) {
	...
	$scope.init = function() {
		...
		$http
				.get('rest/protected/tipoUsuario/getAll')
				.success(
						function(response) {
	...
	$scope.edit = function(event) {
		if (this.editUserForm.$valid) {
			...
			$http.post('rest/protected/users/edit', $scope.requestObject)
					.success(function(response) {
